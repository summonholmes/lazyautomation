...
```bash
$ conda install -c conda-forge schedule -y && conda install -c plotly plotly-orca -y && conda install colorama flake8 jupyter keyring openpyxl pandas plotly psycopg2 psutil scikit-learn scipy seaborn sqlalchemy termcolor virtualenv xlrd yapf -y && conda update --all -y && conda clean --all -y
```
...
```
λ choco install 7zip adb bleachbit choco-cleaner discord epicgameslauncher etcher ffmpeg FiraCode Firefox foobar2000 git gpg4win-light guitar-pro keepassxc kdenlive k-litecodecpackfull msvisualcplusplus2013-redist nano nomacs notepadplusplus protonmailbridge qbittorrent retroarch rufus simplewall slack steam sumatrapdf synctrayzor ungoogled-chromium unxUtils vim vscode youtube-dl -y
```
